...
`Rust`를 경험해 보기 위한 환경을 준비하고, 앞으로 소개할 예제들을 실행해 볼 수 있는 간단한 프로젝트를 만들어 보겠습니다.

## 개발 환경

### Rust 컴파일러 설치

최신 언어 트렌드에 걸맞게 컴파일러 설치와 개발 환경 설정은 매우 간단합니다. `Rust` 언어의 개발 환경을 설치하기 위한 `rustup`이라는 설치 관리자가 있으며, [https://rustup.rs/](https://rustup.rs/)에 접속하면 `rustup` 도구를 설치하는 방법을 안내합니다.

사이트에 접속하시면 사용 중인 운영체제에 따라 시스템별로 다른 메시지나 명령어를 보여 줍니다. 리눅스나 유닉스 계열이라면 다음과 같은 명령어를 안내할 것입니다.
...
중간에 설치 환경 설정을 기본값(`default`)으로 진행할지, 아니면 사용자가 직접 수정할지 물어보는데 1번을 입력하여 기본 환경으로 설치를 진행했습니다. 기본 환경을 선택하면 `rustc` 같은 컴파일러와 `cargo`, `clippy`, `rustfmt` 등 개발에 필수적인 도구들을 자동으로 설치하고 환경 변수까지 설정해 줍니다.
...
`Rust` 언어로 개발할 때 가장 자주 사용하게 되는 도구는 `cargo`입니다. 프로젝트 생성과 빌드, 외부 라이브러리 다운로드 등 프로젝트 관리 전반을 담당합니다. 도움말을 확인해 보면 `new`, `build`, `clean` 등 이름만 봐도 용도를 짐작할 수 있는 명령이 가득합니다.
...
가장 먼저 사용하게 될 명령어는 `cargo new`입니다. 새로운 프로젝트를 생성하는 명령입니다. `Rust`에서는 여러 개의 실행 파일과 라이브러리가 포함된 프로젝트 단위를 '패키지'라고 부릅니다. 그래서 `new` 명령의 설명에 "새로운 `cargo` 패키지를 만든다"는 내용이 있는 이유입니다.
`new` 명령을 다음과 같이 실행하면 새로운 디렉토리와 하위 구조를 자동으로 생성해 줍니다. `Rust` 언어의 패키지 및 프로젝트 관리에 대해서는 나중에 다시 자세히 설명하겠습니다.
...
지금은 개발 환경이 제대로 설치되었는지 확인하는 단계이므로 `cargo`의 모든 명령어를 상세히 다루지는 않겠습니다. `apt`나 `yum` 같은 패키지 관리자를 써 본 경험이 있다면, `cargo help` 메시지만 읽어 봐도 충분히 이해할 수 있을 만큼 직관적이고 강력한 도구입니다. 또한 문법이 간단한 `TOML` 포맷을 사용하므로 금방 적응하실 수 있을 것입니다.
...
`[package]` 섹션의 `name`과 `version`은 말 그대로 패키지의 이름과 버전입니다. `edition` 항목은 `Rust` 언어의 명세 버전(에디션)입니다. 이 책을 쓰는 시점에서 가장 최신 버전은 `2021`이며, 그 이전에는 `2018` 버전이 있었습니다. 지금 단계에서는 `cargo`가 기본으로 지정해 주는 값을 그대로 사용하면 됩니다.
...
`cargo add` 실행 시 `crates.io` 인덱스를 업데이트한다는 메시지가 나오는데, 이는 [https://crates.io/](https://crates.io/)라는 사이트에서 `anyhow` 라이브러리 정보를 가져오기 때문입니다. 해당 사이트에서 검색해 보면 최신 버전을 직접 확인할 수 있습니다.
...
`main` 함수는 프로그램의 시작점입니다. 위의 예제에서는 인자와 반환값이 없지만, 필요에 따라 추가할 수도 있습니다. `println!()`은 문자열을 출력하는 매크로 함수입니다. 함수 이름 끝에 `!`가 붙은 것은 일반 함수가 아닌 매크로로 정의되었음을 의미합니다. `println!`에서 `ln`은 `line`의 약자입니다. `C` 언어의 `printf`를 쓸 때 `\n`을 빠뜨리는 실수를 자주 하곤 하는데, `Rust`는 아예 줄 바꿈이 포함된 `println!`을 제공하여 그런 번거로움을 덜어 줍니다. (물론 `!`를 빠뜨리는 새로운 실수를 하게 될 수도 있습니다.)

출력 형식을 지정할 때는 `{}`를 사용하며, 이는 `Python`과 비슷합니다. `{}` 사이에 형식을 지정하는 표시를 넣을 수도 있는데, `{:x}`는 16진수로 출력하라는 의미입니다.
